...
from project.api.depends import database, price_service_repo

router = APIRouter()


@router.get("/all_price_services", response_model=list[PriceServiceSchema], status_code=status.HTTP_200_OK)
...
@router.get("/price_service/{price_service_id}", response_model=PriceServiceSchema, status_code=status.HTTP_200_OK)
