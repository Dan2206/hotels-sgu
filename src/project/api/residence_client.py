...
from project.api.depends import database, residence_client_repo

router = APIRouter()


@router.get("/all_residence_clients", response_model=list[ResidenceClientSchema], status_code=status.HTTP_200_OK)
...
@router.get("/residence_client/{residence_client_id}", response_model=ResidenceClientSchema, status_code=status.HTTP_200_OK)
...
@router.post("/add_residence_client", response_model=ResidenceClientSchema, status_code=status.HTTP_201_CREATED)
